...
import styles from "../ProgramAdd/ProgramAdd.module.css";
...
interface ProgramJurusanFormData {
  kode_jurusan: string;
  judul: string;
  deskripsi: string;
  thumbnail?: string;
...
interface TambahProgramJurusanProps {
  jurusan: Jurusan;
}

export default function TambahProgramJurusan({
  jurusan,
}: TambahProgramJurusanProps) {
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);
  const [isUploading, setIsUploading] = useState(false);
  const [photoPreview, setPhotoPreview] = useState<string>("");
  const [uploadedImagePath, setUploadedImagePath] = useState<string>("");
  const [formData, setFormData] = useState<ProgramJurusanFormData>({
  ...
  const handleNavigationConfirm = async () => {
    ...
    if (uploadedImagePath) {
      try {
        await fetch(`/api/upload/program-jurusan?path=${uploadedImagePath}`, {
          method: "DELETE",
        });
      } catch (error) {
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch("/api/upload/program-jurusan", {
        method: "POST",
        body: formData,
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/program-jurusan", {
        method: "POST",
        headers: {
      ...
      if (result.success) {
        toast.success("Data program jurusan berhasil disimpan!");
        setHasChanges(false);
        const route = `/program/jurusan/${jurusan.kode.toLowerCase()}`;
        router.push(route);
      } else {
        toast.error(result.message || "Gagal menyimpan data program jurusan");
      }
    } catch (error) {
  ...
  const handleCancelClick = () => {
    if (hasChanges) {
      ...
      router.push(`/program/jurusan/${jurusan.kode.toLowerCase()}`);
    }
  };
        ...
        <div className={styles.cardContainer}>
          <div className={styles.cardHeader}>
            <h5 className={styles.cardTitle}>
              Form Tambah Program {jurusan.nama}
            </h5>
          </div>
