...
import { ProgramJurusan } from "WT/types/program";
import styles from "../../ProgramModal/EditProgramModal/EditProgramModal.module.css";
...
interface EditModalProps {
  isOpen: boolean;
  onClose: () => void;
  program: ProgramJurusan | null;
  onSuccess: () => void;
}

const EditProgramJurusanModal = ({
  isOpen,
  onClose,
  program,
  onSuccess,
  ...
  const form = useForm({
    ...
    onSubmit: async (values) => {
      ...
      const res = await fetch(`/api/program-jurusan/${program.id}`, {
        method: "PUT",
      ...
      if (data.success) {
        if (
          ...
          await fetch(
            `/api/upload/program-jurusan?path=${photoUpload.originalImagePath}`,
            { method: "DELETE" }
          ).catch(console.error);
        }
  ...
  return (
    ...
    >
      <form onSubmit={form.handleSubmit} className={styles.form}>
        ...
        <PhotoUpload
          label="Thumbnail"
          uploadEndpoint="/api/upload/program-jurusan"
          deleteEndpoint="/api/upload/program-jurusan"
...
export default EditProgramJurusanModal;
